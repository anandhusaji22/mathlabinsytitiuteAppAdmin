...
// Helper function to extract bytes from web file format
Uint8List? _getWebFileBytes(String fileText) {
  if (fileText.startsWith("WEB_FILE:")) {
    final firstColonIndex = fileText.indexOf(":");
    final secondColonIndex = fileText.indexOf(":", firstColonIndex + 1);
    if (secondColonIndex > firstColonIndex) {
      final base64Bytes = fileText.substring(secondColonIndex + 1);
          ...
          children: [
            InkWell(
              onTap: () async {
                ...
                if (response != null) {
                  if (kIsWeb) {
                    // Store as "WEB_FILE:filename:base64bytes" format for web
                    final bytes = response.files.single.bytes;
                    final filename = response.files.single.name;
                    final base64Bytes = base64Encode(bytes!);
                    file.text = "WEB_FILE:$filename:$base64Bytes";
            ...
            if (file.text != "")
              Container(
                  width: 200,
                  height: 200,
                  child: (!file.text.contains("https") && !file.text.startsWith("WEB_FILE:"))
                      ? (kIsWeb)
                          ? Image.memory(
                              _getWebFileBytes(file.text) ?? Uint8List.fromList(file.text.codeUnits),
                              fit: BoxFit.cover,
                            )
                          : Image.file(
                              File(file.text),
                      ...
                      : file.text.startsWith("WEB_FILE:")
                          ? Image.memory(
                              _getWebFileBytes(file.text)!,
                              fit: BoxFit.cover,
                            )
                          : Image.network(
                              (file.text),
