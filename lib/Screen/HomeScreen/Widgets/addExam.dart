...
class _AddExamState extends State<AddExam> {
  ...
  void initState() {
    ...
    if (Edata != 1) {
      ...
      pdfLink.text = md.pdf ?? "";
      isActive.text = (Edata["is_active"] == true) ? "1" : "";
    }
    super.initState();
  }
  ...
  EditVideoData() {
    ...
    model.totalMarks = int.parse(totalMarks.text).toInt();
    model.passmark = int.parse(passmark.text).toInt();
    // Explicitly set isActive: true if checkbox is checked ("1"), false otherwise
    model.isActive = isActive.text.trim() == "1";
    print("EditVideoData - isActive checkbox value: '${isActive.text}', setting isActive to: ${model.isActive}");
    model.accessType = int.parse(accessType.text).toInt();
    model.pdf = pdfLink.text;
